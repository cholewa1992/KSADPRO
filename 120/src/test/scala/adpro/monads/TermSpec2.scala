...
class  TermSpec extends FlatSpec with Checkers {
  ...
  val genSafeTerm :Gen[Term] = {
    ...
    val genUnsafeTerm :Gen[Term] = {
      ...
      it should "return 42 from a Consstant " in {
      BasicEvaluator.eval (Consst) shouldBe 42 }
      ...
      it should "answer Return(42) to a Consstant" in
      { ExceptionEvaluator.eval (Consst) shouldBe ExceptionEvaluator.Return(42) }
      ...
      it should "should count no divisions" in check {
        forAll { (n: Int) =>
      StateEvaluator.eval (Consst).step (n) == (42,n) } }
      ...
      it should "return simple result for a Consstant" in {
        val r = OutputEvaluator.eval(Consst)
        r.a shouldBe 42
      ...
      it should "return 42 for a Consstant " in {
      BasicEvaluatorWithMonads.eval (Consst).a shouldBe 42 }
      ...
      it should "should count no divisions" in check {
        forAll { (n: Int) =>
      StateEvaluatorWithMonads.eval (Consst).step (n) == (42,n) } }
      ...
      it should "return simple result for a Consstant" in {
        val r = OutputEvaluatorWithMonads.eval(Consst)
        r.a shouldBe 42
        r.o shouldBe "eval(Cons(42)) <= 42\n"
      }
      it should "throw a scala exception on division by 0" in
